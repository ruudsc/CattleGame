...
void FCustomGitOperations::GetBranches(TArray<FString> &OutBranches, FString &OutCurrentBranch)
{
    OutCurrentBranch.Empty();
    TArray<FString> Results, Errors;
    if (RunGitCommand(TEXT("branch"), TArray<FString>(), TArray<FString>(), Results, Errors))
    {
        for (const FString &Line : Results)
        {
            FString TrimmedLine = Line.TrimStartAndEnd();
            if (TrimmedLine.StartsWith(TEXT("* ")))
            {
                ...
                FString BranchName = TrimmedLine.Replace(TEXT("* "), TEXT(""));
                OutCurrentBranch = BranchName;
                OutBranches.Add(BranchName);
            }
            else
            {
                OutBranches.Add(TrimmedLine);
            }
        }
    }
}
