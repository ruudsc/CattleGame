                        ...
                        .BorderImage(FAppStyle::Get().GetBrush("ToolPanel.GroupBorder"))
                            [SNew(SVerticalBox)
                             ...
                             + SVerticalBox::Slot().AutoHeight().Padding(2)
                                   [SNew(SDropTarget)
                                        .OnAllowDrop_Lambda([](TSharedPtr<FDragDropOperation> DragDropOp)
                                                            { return DragDropOp->IsOfType<FGitFileDragDropOp>(); })
                                        .OnDropped(this, &SCustomGitWindow::OnLocalDrop)
                                        .Content()
                                            [SNew(SButton).Text(LOCTEXT("TabLocal", "Local Changes")).OnClicked(this, &SCustomGitWindow::OnViewLocalClicked)]] +
                             SVerticalBox::Slot().AutoHeight().Padding(2)
                                 [SNew(SDropTarget)
                                      .OnAllowDrop_Lambda([](TSharedPtr<FDragDropOperation> DragDropOp)
                                                          { return DragDropOp->IsOfType<FGitFileDragDropOp>(); })
                                      .OnDropped(this, &SCustomGitWindow::OnStagedDrop)
                                      .Content()
                                          [SNew(SButton).Text(LOCTEXT("TabStaged", "Staged Files")).OnClicked(this, &SCustomGitWindow::OnViewStagedClicked)]] +
                             SVerticalBox::Slot().AutoHeight().Padding(2)[SNew(SButton).Text(LOCTEXT("TabLocked", "Locked Files")).OnClicked(this, &SCustomGitWindow::OnViewLockedClicked)]
                             ...
                             SVerticalBox::Slot().FillHeight(0.5f)
                                 [SAssignNew(BranchListView, SListView<TSharedPtr<FString>>)
                             ...
                             + SVerticalBox::Slot().AutoHeight().Padding(2, 5)
                                   [SNew(SHorizontalBox) + SHorizontalBox::Slot().FillWidth(1.0f)[SAssignNew(NewBranchNameInput, SEditableTextBox).HintText(LOCTEXT("NewBranchHint", "New Branch Name"))] + SHorizontalBox::Slot().AutoWidth().Padding(2, 0, 0, 0)[SNew(SButton).Text(LOCTEXT("Create", "+")).OnClicked(this, &SCustomGitWindow::OnCreateBranchClicked) // Small button
    ]] + SVerticalBox::Slot().AutoHeight().Padding(0, 10, 0, 5)[SNew(SSeparator).Orientation(Orient_Horizontal)]
                             ...
                             + SVerticalBox::Slot().AutoHeight().Padding(2)
                                   [SNew(SButton)
                                        .Text(LOCTEXT("SyncBtn", "Sync (Pull/Push)"))
                                        .OnClicked(this, &SCustomGitWindow::OnSyncClicked)] +
                             SVerticalBox::Slot().AutoHeight().Padding(2)
                                 [SNew(SButton)
                                      .Text(LOCTEXT("ResetHardBtn", "Reset Hard to Origin"))
                                      .ButtonColorAndOpacity(FLinearColor(0.8f, 0.2f, 0.2f, 1.0f))
                                      .OnClicked(this, &SCustomGitWindow::OnResetHardClicked)]]]
...
void SCustomGitWindow::UpdateBranchList()
{
    ...
    {
        BranchList.Add(MakeShareable(new FString(Branch)));
    }
...
TSharedRef<ITableRow> SCustomGitWindow::OnGenerateBranchRow(TSharedPtr<FString> InItem, const TSharedRef<STableViewBase> &OwnerTable)
{
    bool bIsCurrentBranch = (*InItem == CurrentBranchName);

    return SNew(STableRow<TSharedPtr<FString>>, OwnerTable)
        [SNew(SHorizontalBox) + SHorizontalBox::Slot().AutoWidth().Padding(5, 0, 0, 0)[SNew(STextBlock).Text(bIsCurrentBranch ? FText::FromString(TEXT("*")) : FText::GetEmpty()).Font(FCoreStyle::GetDefaultFontStyle("Bold", 10))] + SHorizontalBox::Slot().FillWidth(1.0f).Padding(2, 0, 5, 0)[SNew(STextBlock).Text(FText::FromString(*InItem)).Font(bIsCurrentBranch ? FCoreStyle::GetDefaultFontStyle("Bold", 10) : FCoreStyle::GetDefaultFontStyle("Regular", 10))]];
}

TSharedPtr<SWidget> SCustomGitWindow::OnOpenBranchContextMenu()
{
    TArray<TSharedPtr<FString>> SelectedBranches = BranchListView->GetSelectedItems();
    if (SelectedBranches.Num() == 0)
    {
        return nullptr;
    }

    FString BranchName = *SelectedBranches[0];

    // Check if branch has upstream tracking
    TArray<FString> Results, Errors;
    bool bHasUpstream = false;
    FCustomGitOperations::RunGitCommand(TEXT("config"), {FString::Printf(TEXT("branch.%s.remote"), *BranchName)}, {}, Results, Errors);
    bHasUpstream = (Results.Num() > 0 && !Results[0].IsEmpty());
    ...
    MenuBuilder.AddMenuEntry(
        ...
        FUIAction(
            FExecuteAction::CreateLambda([this, SelectedBranches]()
                ...
                {
                    FString Error;
                    FCustomGitOperations::SwitchBranch(*SelectedBranches[0], Error);
                    OnRefreshClicked();
                } })));

    MenuBuilder.AddSeparator();

    // Show "Push branch to origin" only if no upstream exists
    if (!bHasUpstream)
    {
        MenuBuilder.AddMenuEntry(
            ...
            FExecuteAction::CreateLambda([this, SelectedBranches]()
                ...
                if (SelectedBranches.Num() > 0)
                {
                    FText Message = FText::Format(
                        ...
                        FText::FromString(*SelectedBranches[0]));
                    EAppReturnType::Type Result = FMessageDialog::Open(EAppMsgType::YesNo, Message);
                    if (Result == EAppReturnType::Yes)
                    {
                        TArray<FString> Results, Errors;
                        FCustomGitOperations::RunGitCommand(TEXT("branch"), {TEXT("-d"), *SelectedBranches[0]}, {}, Results, Errors);
                        UpdateBranchList();
                    }
                } })));
